 ...
 * $Id: 6f2f4dfaec9c7232e8df96a9ca673e3eebbe8673 $
 *
...
//BTH metrics start here.
FILE *out_one;
//BTH metrics end here.

//BTH log rebuffering events here.
void LogITEsout(char *whatever){
  ...
  sprintf(current_timeOK, "%ld.%06ld//%s",tv.tv_sec, millisecondsOK,whatever);
  //sprintf(directory,"%s_%s","/sdcard/vlcplayer/vlc_rebuffer",dandroidDeviceID);
  sprintf(directory,"%s","/sdcard/vlcplayer/vlc_rebuffer");
...
static void EsOutDecodersStopBuffering( es_out_t *out, bool b_forced )
{
    ...
    if( i_stream_duration <= i_buffering_duration && !b_forced )
    {
        ...
        if( p_sys->i_prev_stream_level != i_level )
        {
            ...
            char eventType[20]="Buffering";
            LogITEsout(eventType);
        }
    ...
    char eventType[30]="BufferingCompleted";//BTH patch
    LogITEsout(eventType);//BTH patch
