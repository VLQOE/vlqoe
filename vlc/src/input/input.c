 ...
 * $Id: af7eaacb8f20264e81acc22dda3d6390f772de05 $
 *
...
void LogITInput(char *whateverInput){
  /// BTH measurement patch starts here
  struct timeval tvInput;
  long millisecondsGenericInput;

  gettimeofday(&tvInput,NULL);
  char current_timeGenericInput[50];
  char reportsGenericInput[20];
  char directoryInput[50];

  millisecondsGenericInput=tvInput.tv_usec;

  long millisecondsOKInput;
  gettimeofday(&tvInput,NULL);
  char current_timeOKInput[50];
  char reportsOKInput[20];
  millisecondsOKInput=tvInput.tv_usec;
  sprintf(current_timeOKInput, "%ld.%06ld//%s",tvInput.tv_sec, millisecondsOKInput,whateverInput);
  sprintf(directoryInput,"%s_%s","/sdcard/vlcplayer/vlc_source",inputandroidDeviceID);

  out_input = fopen(directoryInput,"a");
  fprintf(out_input, "%s\n", current_timeOKInput);
  fclose(out_input);
                               ...
                               input_resource_t *p_resource )
{
    ...
    inputfp = fopen("/sdcard/vlcplayer/vlc_device", inputmode);
    fscanf(inputfp, "%s", inputandroidDeviceID);
    fclose(inputfp);

    char* videoSourceName=psz_name;
    LogITInput(videoSourceName);
