...
FILE *out_esout_one;
FILE *out_esout_two;
FILE *out_esout_three;
FILE *out_esout_four;
FILE *our_esout_five;

FILE *dfp;
char *dmode = "r";
char dandroidDeviceID[30];
...
//    msg_Warn(p_input,"EsOutNew_%ld",i_rate);

    dfp = fopen("/sdcard/vlcplayer/vlc_device", dmode);
    fscanf(dfp, "%s", dandroidDeviceID);
    fclose(dfp);
