 ...
 * $Id: f9e52cdb377c9b3ca9c06accb068b2e27cc47199 $
 *
...
int framesPUT=0;
int framesGeneric=0;
long prevActualtimeOK=0;
long prevActualtimePUT=0;
long prevActualtimeGeneric=0;
int lastClctedframes=0;
char androidDeviceID[30];

FILE *ifp;
char *mode = "r";


//BTH initialisation ends here...
...
void LogIT(vout_thread_t *vout,char* location){
  /// BTH measurement patch starts here
  struct timeval tv;
  long millisecondsGeneric;
  gettimeofday(&tv,NULL);
  char current_timeGeneric[50];
  char reportsGeneric[20];	
  char directory[50];
  
  millisecondsGeneric=tv.tv_usec;

  long millisecondsOK;
  gettimeofday(&tv,NULL);
  char current_timeOK[50];
  char reportsOK[20];
  millisecondsOK=tv.tv_usec;
  sprintf(current_timeOK, "%s//%ld.%06ld",location,tv.tv_sec, millisecondsOK);
//  msg_Warn(vout, "%s",current_timeOK);

  sprintf(directory,"%s_%s","/sdcard/vlcplayer/vlc_frames",androidDeviceID);
  
  out_one = fopen(directory,"a");
  fprintf(out_one, "%s\n", current_timeOK);
  fclose(out_one);
...
{
  //  msg_Warn(vout, "VoutGetDisplayCfg_%s",androidDeviceID);
    ifp = fopen("/sdcard/vlcplayer/vlc_device", mode);
    fscanf(ifp, "%s", androidDeviceID);
    fclose(ifp); 
    /* Load configuration */
