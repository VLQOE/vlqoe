...
public class MainActivity extends SherlockFragmentActivity {
	...
	try {  
	NeighboringCellInfo thisCell = neighbors.get(i);  
	int thisNeighCID = thisCell.getCid();  
	threeGSignalStr  = thisCell.getRssi();  
	Log.i(TAG," "+thisNeighCID+" - "+threeGSignalStr);  
 	}
	catch (NumberFormatException e) {  
   	e.printStackTrace();
   	NeighboringCellInfo thisCell = neighbors.get(i);  
   	Log.i(TAG,neighbors.toString());  
 	}  
	}  
        ...
        writeToFile("vlc_context_"+myDeviceID,"");
        writeToFile("vlc_ratings_"+myDeviceID,"");
        writeToFileSingle("vlc_device","");
        writeToFileSingle("vlc_device",myDeviceID);
        writeToFile("vlc_wifiRSSI_"+myDeviceID,"");
        writeToFile("vlc_source_"+myDeviceID,"");
	writeToFile("vlc_rebuffer_"+myDeviceID,"");
        writeToFile("vlc_nettype_"+myDeviceID,"");
        writeToFile("vlc_location_"+myDeviceID,"");
        writeToFile("vlc_freeze_"+myDeviceID,"");
        writeToFile("vlc_battery_"+myDeviceID,"");
        writeToFile("vlc_nettype_"+myDeviceID,"");
	writeToFile("vlc_controls_"+myDeviceID,"");
        writeToFile("vlc_content_"+myDeviceID,"");
 	writeToFile("vlc_application_"+myDeviceID,"");
	writeToFile("vlc_cellular_"+myDeviceID,"");
        writeToFile("vlc_cellular_"+myDeviceID,threeGSignalStr+"");
	
	wifi = (WifiManager)this.getSystemService(Context.WIFI_SERVICE);
	WifiInfo info = wifi.getConnectionInfo();
	writeToFile("vlc_wifiRSSI_"+myDeviceID,info.getRssi()+","+getClock()+"\n");
	...
	alert.setPositiveButton("Submit",new DialogInterface.OnClickListener() {
		public void onClick(DialogInterface dialog, int whichbutton) {
			...
			writeToFile("vlc_context_"+myDeviceID,locationmobility+","+getClock()+"\n");
		}
	});

	alert.setNegativeButton("Cancel", new DialogInterface.OnClickListener(){
		public void onClick(DialogInterface dialog, int whichbutton){
			writeToFile("vlc_context_"+myDeviceID,"cancelled,"+getClock()+"\n");
		}
	});
...
//BTH measurement patch starts here....
  ...
  public String getClock(){
    ...
    protected void onDestroy() {
        super.onDestroy();
        writeToFile("vlc_application_"+myDeviceID,"closed"+","+getClock()+"\n");
    ...
    public void onBackPressed() {
        writeToFile("vlc_application_"+myDeviceID,"backPressed"+","+getClock()+"\n");
    ...
    private final BroadcastReceiver messageReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(WifiManager.RSSI_CHANGED_ACTION)) {
              	int new_signal_strength = intent.getIntExtra(WifiManager.EXTRA_NEW_RSSI, 0);
             	 String message = String.format("Changed to : %s",new_signal_strength);
             	 Log.d(TAG, "onReceive() message: " + message);
              	writeToFile("vlc_wifiRSSI_"+myDeviceID,new_signal_strength+","+getClock()+"\n");
	      }
            ...
            public void onClick(DialogInterface dialog, int button) {
                ...
                VLCCallbackTask task = new VLCCallbackTask(MainActivity.this)
                {
                    ...
                    public void run() {
                      ...
                      writeToFile("vlc_content_"+myDeviceID,s+","+getClock()+"\n");
