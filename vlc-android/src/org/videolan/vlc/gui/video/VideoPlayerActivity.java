...
public class VideoPlayerActivity extends Activity implements IVideoPlayer {
 ...
 private Button mFreeze,mMosOne, mMosTwo,mMosThree, mMosFour, mMosFive;
    ...
    private static int updateIntervalGPS=3;
    private static int updateIntervalNetwork=5;
    private static float minDistGPS=10;
    private static float minDistNet=100;

    long mReceived=0;
    long mTransmitted=0;

    long  prevUidRxBytes=0;
    long  prevUidRxPackets=0;
    long  prevUidTcpRxBytes=0;
    long  prevUidTcpRxSegments=0;
    long  prevUidTcpTxBytes=0;
    long  prevUidTcpTxSegments=0;
    long  prevUidTxBytes=0;
    long  prevUidTxPackets=0;
    long  prevUidUdpRxBytes=0;
    long  prevUidUdpRxPackets=0;
    long  prevUidUdpTxBytes=0;
    long  prevUidUdpTxPackets=0;

    long prevRxBytes=0;
    long prevRxPackets=0;
    long prevTxBytes=0;
    long prevTxPackets=0;

    GPSModule mGPS;

    FileOutputStream fos;
    RandomAccessFile cpuStatsFile;
    long initTx=0;
    long initRx=0;
    Handler myPacketHandler = new Handler();
    ...
    int clockCounter=0;long timeMillis=0;
    // size of the video
	...
	//BTH measurement starts here...

	File rootPath = new File(Environment.getExternalStorageDirectory(), "vlcplayer");
        if(!rootPath.exists()) {
          try{
		Process pr= Runtime.getRuntime().exec("mkdir /sdcard/vlcplayer");
		pr.waitFor();
	  }
	  catch(Exception e){}  
	}
//Commented out for energy measurements

	TelephonyManager tm = (TelephonyManager)this.getSystemService(Context.TELEPHONY_SERVICE);

        myDeviceID = tm.getDeviceId();

	mGPS=new GPSModule(this);
	
	if(mGPS.canGetLocation()){
		double lat=mGPS.getLatitude();
		double lngt=mGPS.getLongitude();
        writeToFile("vlc_location_"+myDeviceID,lat+","+lngt+","+getClock()+"\n");   //write MOS to file;
//		Log.i(TAG,"Location:"+lat+":"+lngt);
	}
	else {mGPS.showSettingsAlert();}

	writeToFile("vlc_screen_"+myDeviceID,"orientation,"+getScreenOrientation()+","+getClock()+"\n");

	String simOperatorName = tm.getSimOperatorName();
	int networkType = tm.getNetworkType();
	int dataState  = tm.getDataState();
	writeToFile("vlc_nettype_"+myDeviceID,simOperatorName+","+networkType+","+dataState+","+getClock()+"\n");

	timeMillis=System.currentTimeMillis();
	writeToFile("vlc_ratings_"+myDeviceID,"");
	writeToFile("vlc_freeze_"+myDeviceID,"");
	//BTH measurement ends here....
	writeToFile("vlc_controls_"+myDeviceID,"");
	writeToFile("vlc_screen_"+myDeviceID,"");
        writeToFile("vlc_application_"+myDeviceID,"VideoPaneStarted"+","+getClock()+"\n");
	...
	mFreeze=(Button) findViewById(R.id.player_overlay_freeze);
       mMosOne=(Button) findViewById(R.id.player_overlay_mosone);
       mMosTwo=(Button) findViewById(R.id.player_overlay_mostwo);
       mMosThree=(Button) findViewById(R.id.player_overlay_mosthree);
       mMosFour=(Button) findViewById(R.id.player_overlay_mosfour);
       mMosFive=(Button) findViewById(R.id.player_overlay_mosfive);

        mFreeze.setOnClickListener(new OnClickListener(){
                public void onClick(View view){
                     writeToFile("vlc_freeze_"+myDeviceID,"0,"+getClock()+"\n");   //write MOS to file;

        }
        });


        mMosOne.setOnClickListener(new OnClickListener(){
		public void onClick(View view){
                     writeToFile("vlc_ratings_"+myDeviceID,"1,"+getClock()+"\n");   //write MOS to file;

	}
	});  
    mMosTwo.setOnClickListener(new OnClickListener(){
                public void onClick(View view){
                     writeToFile("vlc_ratings_"+myDeviceID,"2,"+getClock()+"\n");   //write MOS to file;

        }
        }); 

    mMosThree.setOnClickListener(new OnClickListener(){
                public void onClick(View view){
                     writeToFile("vlc_ratings_"+myDeviceID,"3,"+getClock()+"\n");   //write MOS to file;

        }
        }); 

    mMosFour.setOnClickListener(new OnClickListener(){
                public void onClick(View view){
                     writeToFile("vlc_ratings_"+myDeviceID,"4,"+getClock()+"\n");   //write MOS to file;

        }
        }); 


    mMosFive.setOnClickListener(new OnClickListener(){
                public void onClick(View view){
                     writeToFile("vlc_ratings_"+myDeviceID,"5,"+getClock()+"\n");   //write MOS to file;
...
//BTH measurement patch starts here....
  ...
  public String tail( File file, int lines) {
	...
	initRx=TrafficStats.getTotalRxBytes();
	initTx=TrafficStats.getTotalTxBytes();

	if(initRx!=TrafficStats.UNSUPPORTED&&initTx!=TrafficStats.UNSUPPORTED)
	{
		myPacketHandler.postDelayed(myPacketRunnable,1000);
	}
    }


    public final Runnable myPacketRunnable = new Runnable(){
	public void run(){
	long currentRXBytes=TrafficStats.getTotalRxBytes()-initRx;
	long currentTXBytes=TrafficStats.getTotalTxBytes()-initTx;
	
	myPacketHandler.postDelayed(myPacketRunnable,1000);
   	writeToFile("vlc_packets_"+myDeviceID,currentRXBytes+","+currentTXBytes+","+getClock()+"\n");
	}
    };
 ...
 public void writeToFileSingle(String filename, String data){
        if(!data.contentEquals("null")){
           try{
                File file = new File("/sdcard/vlcplayer/"+filename);
                boolean exist = file.createNewFile();
...
//BTH measurement patch ends here...
    ...
    protected void onDestroy() {
        ...
        writeToFile("vlc_application_"+myDeviceID,"VideoPaneBackPressed"+","+getClock()+"\n");
    ...
    protected void onResume() {
        super.onResume();
        writeToFile("vlc_application_"+myDeviceID,"VideoPaneResumed"+","+getClock()+"\n");
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver()
    {
	    ...
	    int  batteryHealth= intent.getIntExtra(BatteryManager.EXTRA_HEALTH,0);
            ...
            writeToFile("vlc_battery_"+myDeviceID,batteryLevel+","+batteryScale+","+batteryPlugged+","+batteryStatus+","+batteryTechnology+","+batteryTemperature+","+batteryVoltage+","+batteryHealth+","+getClock()+"\n");            
                if (batteryLevel >= 50)
...
//        int cellularSignal = signalStrength.getGsmSignalStrength();
...
//}
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        writeToFile("vlc_controls_"+myDeviceID,"touch,"+getClock()+"\n");   //write MOS to file;
    ...
    private void doSeekTouch(float coef, float gesturesize, boolean seek) {
        ...
        if (seek)
	        ...
	        writeToFile("vlc_controls_"+myDeviceID,"seek,"+getClock()+"\n");   //write MOS to file;
	}
    ...
    private void doBrightnessTouch(float y_changed) {
        ...
        showInfo(getString(R.string.brightness) + '\u00A0' + Math.round(lp.screenBrightness*15),1000);
	    	writeToFile("vlc_screen_"+myDeviceID,"brightness,"+lp.screenBrightness+","+getClock()+"\n");    
}
    ...
    */
    ...
    private void play() {
        ...
        mSurface.setKeepScreenOn(true);
            writeToFile("vlc_controls_"+myDeviceID,"play,"+getClock()+"\n");   //write MOS to file;
    }
    ...
    private void pause() {
        mLibVLC.pause();
        mSurface.setKeepScreenOn(false);
        writeToFile("vlc_controls_"+myDeviceID,"pause,"+getClock()+"\n");   //write MOS to file;
    }
